...
export function ConnectionStatusIndicator({
  ...
  const pollingInfo = useDownloadWebSocket();

  const getStatusConfig = () => {
    // Check if polling is active to modify the display only when showPollingMode is true
    const isPollingActive = showPollingMode ? pollingInfo.isPolling : false;
    const _pollingReason = showPollingMode ? pollingInfo.pollingReason : null;
  ...
  const formatConnectionDuration = () => {
    ...
    const duration =
      connectionMetadata.connectionDuration ||
      Date.now() - connectionMetadata.lastConnected.getTime();
    ...
    <div className="flex flex-col items-end">
      ...
      {showDetails && (
        <div className="flex flex-col items-end space-y-1 mt-1">
            ...
            <>
              {pollingInfo.isPolling && (
                <div className="flex items-center space-x-1">
                  ...
                  <span className="text-xs text-blue-600">
                    Polling every{" "}
                    {Math.round(pollingInfo.pollingInterval / 1000)}s
                  </span>
                </div>
              )}
              ...
              {pollingInfo.lastPollingSuccess && (
                <span className="text-xs text-muted-foreground">
                  Last update:{" "}
                  {Math.floor(
                    (Date.now() - pollingInfo.lastPollingSuccess) / 1000
                  )}
                  s ago
                </span>
              )}
          ...
          {isConnected &&
            connectionMetadata.connectionQuality !== "unknown" && (
          ...
          {isConnected &&
            connectionMetadata.isHeartbeatActive &&
            connectionMetadata.lastPong && (
              <span className="text-xs text-muted-foreground">
                Last ping:{" "}
                {Math.floor(
                  (Date.now() - connectionMetadata.lastPong.getTime()) / 1000
                )}
                s ago
              </span>
            )}
          ...
          {!isConnected && connectionMetadata.lastErrorTime && (
            <span className="text-xs text-muted-foreground">
              Last error:{" "}
              {Math.floor(
                (Date.now() - connectionMetadata.lastErrorTime.getTime()) / 1000
              )}
              s ago
            </span>
          )}
