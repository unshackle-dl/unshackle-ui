...
// evaluated until their route is first requested, so anything long-lived — the Phase 2
// poller above all — has to be started from here or it silently never runs on an idle
// server. Such registration must also be idempotent: dev re-evaluates this module on HMR,
// and `npm run dev` does not evaluate it at all until the first HTTP request, so an idle
// dev server is no evidence either way.
//
...
// stack, rather than as a 500 on whichever tracking request happens to arrive first.
getDb();
