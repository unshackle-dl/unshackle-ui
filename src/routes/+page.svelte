<script lang="ts">
	...
	const profileQs = $derived(profile ? `?profile=${encodeURIComponent(profile)}` : '');
	...
	async function openDirect() {
		...
		opening = true;
		try {
			await goto(`/title/${service}/${encodeURIComponent(id)}${profileQs}`);
		} finally {
			opening = false;
		}
	}
...
<form onsubmit={search} class="mt-6 flex flex-wrap gap-3">
	<div class="relative">
		<select
			bind:value={service}
			onchange={() => (profile = '')}
	...
	{:else}
		...
		>
			{#each results as r (r.id)}
				<a
					href="/title/{service}/{encodeURIComponent(r.id)}{profileQs}"
