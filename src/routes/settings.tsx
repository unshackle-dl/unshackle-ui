...
function SettingsPage() {
	...
	const sans = useSansFont();
	const mono = useMonoFont();
	...
	return (
		<>
			...
			{server.data && (
				<>
					<h2 className="mt-10 text-lg font-semibold tracking-tight">Server</h2>
					<p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
							...
							<div className="sm:col-span-2">
								...
								<dd className="mt-1 flex flex-wrap gap-1.5">
									{server.data.serve.services === null ? (
										...
										server.data.serve.services.map((s) => <Badge key={s}>{mask.service(s)}</Badge>)
									)}
								</dd>
							</div>
						</dl>
					...
					<h2 className="mt-10 text-lg font-semibold tracking-tight">Maintenance</h2>
					<p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
						One-off server housekeeping. Clearing is destructive and asks to confirm.
					</p>

					<Card className="mt-6 max-w-2xl divide-y divide-neutral-100 p-0 dark:divide-neutral-800">
								...
								<div className="flex shrink-0 items-center gap-2">
									{tempOp.kind === 'confirm' ? (
										<>
											...
											<Button variant="secondary" onClick={() => setOp('temp', { kind: 'idle' })}>
												Cancel
											</Button>
										</>
									) : (
			...
			<h2 className="mt-10 text-lg font-semibold tracking-tight">Appearance</h2>
			<p className="mt-1 text-sm text-neutral-500 dark:text-neutral-400">
					...
					<div>
						...
						<div className="relative mt-1.5">
							<select
								id="sansFont"
								value={sans}
								onChange={(e) => sansFont.set(e.target.value)}
								className="w-full appearance-none rounded-lg border border-neutral-200 bg-white py-2 pr-9 pl-3 text-sm text-neutral-900 focus:border-accent-500 focus:ring-2 focus:ring-accent-500/30 focus:outline-none dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-100"
							>
								{SANS_FONTS.map((f) => (
					...
					<div>
						...
						<div className="relative mt-1.5">
							<select
								id="monoFont"
								value={mono}
								onChange={(e) => monoFont.set(e.target.value)}
								className="w-full appearance-none rounded-lg border border-neutral-200 bg-white py-2 pr-9 pl-3 font-mono text-sm text-neutral-900 focus:border-accent-500 focus:ring-2 focus:ring-accent-500/30 focus:outline-none dark:border-neutral-700 dark:bg-neutral-900 dark:text-neutral-100"
							>
								{MONO_FONTS.map((f) => (
									<option key={f.name} value={f.name} style={{ fontFamily: f.stack }}>
										{f.name}
